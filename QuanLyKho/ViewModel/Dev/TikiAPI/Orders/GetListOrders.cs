...
namespace QuanLyKho.ViewModel.Dev.TikiAPI.Orders
{
    ...
    class GetListOrders
    {
        ...
        /// <summary>
        /// Lấy được đơn hàng cần xác nhận còn hàng của 1 shop
        /// </summary>
        /// <param name="configApp">shop</param>
        /// <returns>Danh sách đơn hàng. Rỗng nếu không lấy thành công</returns>
        static public List<Order> GetListOrderNeedAvailabilityConfirmationOneShop(TikiConfigApp configApp)
        {
            ////GET /integration/v2/orders?status=queueing&item_inventory_type=backorder&item_confirmation_status=waiting
            ...
            //IRestResponse response = CommonTikiAPI.ExcuteRequestWithRefreshAccesToken(client, request, configApp);
            ...
            try
            {
                ListOrders lsOrder = JsonConvert.DeserializeObject<ListOrders>(json);
                return lsOrder.data;
                //return null;
            }
            catch(Exception ex)
        ...
        /// <summary>
        /// Lấy được đơn hàng cần xác nhận còn hàng của nhiều shop
        /// </summary>
        /// <param name="listConfigApp">danh sách shop</param>
        /// <returns>Danh sách đơn hàng. Rỗng nếu không lấy thành công</returns>
        static public List<Order> GetListOrderNeedAvailabilityConfirmationAllShops(List<TikiConfigApp> listConfigApp)
        {
            List<Order> lsOrder = new List<Order>();
            int num = listConfigApp.Count();
            for(int i = 0; i < num; i++)
            {
                lsOrder.AddRange(GetListOrderNeedAvailabilityConfirmationOneShop(listConfigApp[i]));
            }
            return lsOrder;
        }
    }
